...
| Emoji in card text | `:heart:` `:smile:` `:smileb:` `:sparkle:` `:diamond:` `:note:` `:notes:` `:star:` shortcodes | Only these 8 render correctly — see constraint below. Each has a default press-animation (heart beats, smile alternates, etc.) on cards using the bounce layout — see `remote-api-spec.md` |
