...
The device polls this site's endpoint on its own schedule (default every 60s) and reacts to whatever it finds. The site's only job is to serve the right JSON and let you edit it through a UI instead of hand-writing JSON.
...
The site needs to persist one "current state" blob — this is what gets served as JSON on every device poll:
...
- Match whatever `REMOTE_API_KEY` you put in the device's `RemoteApi.h` (gitignored, never commit it) — the site needs to check for this exact value on the endpoint the device polls, and reject anything else.
...
This is now built (optional, off by default): if `RemoteApi.h`'s `REMOTE_HEARTBEAT_URL` is configured, the device POSTs a small status ping every poll cycle, in the same WiFi session as the regular state fetch:

```json
{"currentCard": 0, "nightMode": false, "gameMode": false, "uptimeSec": 184320}
```

Your site needs one endpoint that accepts this POST (checked against the same `X-Device-Key` header) and just saves it with a timestamp — that timestamp is what "last seen 2 min ago" is built from. The device doesn't care about the response; treat it as fire-and-forget. Full contract in `remote-api-spec.md`'s Heartbeat section.
...
**The snag:** the device firmware only speaks HTTPS (`WiFiClientSecure`), not plain HTTP. Getting a phone to serve real, working HTTPS locally means generating and trusting a certificate, which is genuinely fiddly even for a developer — not a "just run a script" thing. Plain HTTP (no TLS at all) would be much easier to stand up locally, but the firmware doesn't support it right now, and switching would mean the shared-secret header travels unencrypted — an acceptable tradeoff on a private home/hotspot network, not something I'd want to silently ship as the default.

**If you still want this specifically:** the realistic path is running Termux (a Linux terminal app for Android) with a small Python/Node HTTP server, and the ESP32 pointed at that phone's local IP. On a phone's own hotspot, the phone itself is usually reachable at a predictable address (Android hotspots commonly use `192.168.43.1`), which helps. This needs the firmware's `WiFiClientSecure` swapped for a plain `WiFiClient` (or made to support both) — a real but contained code change, not something I'd do speculatively without confirming you want to go this route.

**What I'd actually recommend instead:** a real (still free-tier) hosted backend — Render, Railway, Fly.io, a small Vercel/Cloudflare function, or even a Google Apps Script Web App (free, zero server management, HTTPS built in). This sidesteps the local-hosting problem entirely, works regardless of which WiFi network the device is on at any given moment (relevant if it isn't always on the same network you are), and doesn't require a phone to be running a background server whenever you want to send a message — which would otherwise cost *that* device battery too. The "local app" instinct makes sense for keeping this self-contained and not dependent on a third party, but a free-tier host achieves the same independence without the TLS problem.
...
This is the concrete answer to "should this be an app" from the earlier local-hosting discussion in §8 — a PWA gets the home-screen/full-screen feel without a separate codebase, app-store account, or the local-HTTPS problem that a real native+local-server setup would still have to solve.
